...
```{r}
wynik <- na.omit(x1)
str(wynik, 1) # structure - wyswietla cala strukture
attr(wynik, "na.action") 
```

```{r}
x2 <- c(11, 22, 33, NA, NaN)
x2
```
...
dane <- read.csv("./data/plik-przyklad.csv",
                 na.strings = c("NULL", "NA", "N/D", "Brak danych", "", "-"))
head(dane)
```
...
```{r, eval = FALSE}
with(dane, cor(wzrost, ocena, use="complete.obs"))
```
...
```{r}
data$AttendanceRate <- as.numeric(data$AttendanceRate)
typeof(data$AttendanceRate)
```

```{r}
mean(data$AttendanceRate, na.rm = T)
```

```{r}
data$MathScore <- as.numeric(data$MathScore)
typeof(data$MathScore)
```

```{r}
median(data$MathScore, na.rm = T)
```

```{r, eval = FALSE}
with(data, cor(data$AttendanceRate, data$MathScore, use="complete.obs"))
```

```{r}
data_NA <- read.csv("./data/exercise2.csv",
                    na.strings = c("NULL", "NA", "-1", "-9"))
head(data_NA)
nrow(data_NA, na.rm=T)
```
